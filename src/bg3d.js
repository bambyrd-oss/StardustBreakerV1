...
// building depth bands: nearer band = storefront row, farther = mid + skyline
const BANDS = [
  { z:-13, wMul:1.0, hBase:11, hVar:8,  depth:9, store:true,  spacing:11 },
  { z:-32, wMul:1.1, hBase:15, hVar:12, depth:10, store:false, spacing:13 },
  { z:-56, wMul:1.2, hBase:17, hVar:14, depth:11, store:false, spacing:16 },
  { z:-92, wMul:1.4, hBase:19, hVar:17, depth:12, store:false, spacing:21, skyline:true },
];
...
function buildColumn(band, baseX, seed){
  const w = (5 + hash(seed)*6) * band.wMul;
...
for(const band of BANDS){
  for(let x=-SPAN/2; x<SPAN/2; x+=band.spacing){
    world.push(buildColumn(band, x + (hash(colSeed)-0.5)*2, colSeed));
    colSeed++;
  }
}
