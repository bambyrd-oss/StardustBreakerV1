...
(function(){
  ...
  for(let k=0;k<N;k++){
    ...
    // a fire escape every third segment: platforms + angled runs
    if(k%3===1){ const fx=(R(13)-0.5)*10;
      for(let f=0;f<3;f++){ const py=4+f*3.2;
        const p=new THREE.Mesh(boxGeo, steel); p.scale.set(4.6,0.16,0.9); p.position.set(fx,py,-8.0); g.add(p);
        const s=new THREE.Mesh(boxGeo, steel); s.scale.set(0.9,3.2,0.14); s.position.set(fx+(f%2?2.1:-2.1),py-1.6,-8.0); s.rotation.z=(f%2?-1:1)*0.55; g.add(s);
        const rail=new THREE.Mesh(boxGeo, steel); rail.scale.set(4.6,0.06,0.06); rail.position.set(fx,py+0.9,-7.6); g.add(rail); } }
