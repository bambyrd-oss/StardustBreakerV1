...
function buildStoreMesh(){
  // local x=0 is the DOOR (the game's trigger point); the body hangs left of it like the old facade
  const g=new THREE.Group();
  const brick=brickTexture(); brick.repeat.set(3,4);
  const body=new THREE.Mesh(boxGeo, toonMat({ map:brick, roughness:.95 }));
  body.scale.set(STORE_W,STORE_H,STORE_D); body.position.set(-2.2,STORE_H/2,-STORE_D/2);
  body.castShadow=true; body.receiveShadow=true; g.add(body);
  const ol=new THREE.Mesh(boxGeo, OUTLINE_MAT); ol.scale.set(STORE_W,STORE_H+0.7,STORE_D+0.7); ol.position.copy(body.position); g.add(ol);
  const cor=new THREE.Mesh(boxGeo, toonMat({ color:'#d9c6a8', roughness:.9 }));
  cor.scale.set(STORE_W*1.06,0.6,STORE_D*1.1); cor.position.set(-2.2,STORE_H-0.1,-STORE_D/2); g.add(cor);
  for(let i=0;i<3;i++){ const wm=new THREE.Mesh(new THREE.PlaneGeometry(1.7,1.5),
    ...
    wm.position.set(-5.2+i*3, 7.6, 0.06); g.add(wm); }
  // pink striped awning over the shopfront
  ...
  aw.scale.set(STORE_W*0.98,0.45,2.0); aw.position.set(-2.2,5.6,0.7); aw.rotation.x=-0.3; g.add(aw);
  ...
  win.position.set(-4.5,2.5,0.06); g.add(win); g.userData.win=win;
...
let bgTick=0;
  ...
  function stepBg(sc, storeXs){
    scroll=sc; bgTick++;
    // park the pooled store buildings at the on-screen store doors; hide the rest
    ...
    for(let i=0;i<storePool.length;i++){ const s=storePool[i];
      if(i<xs.length){ s.visible=true; s.position.x=xs[i]-scroll; s.userData.win.material.map= lit?OPEN_ON:OPEN_OFF; }
      else s.visible=false; }
    const gaps=xs.map(x=>x-scroll-2.2);   // store body centres, in screen space (door is +2.2 off-centre)
    for(const b of world){ const sx=b.baseX-scroll; if(sx<-SPAN/2)b.baseX+=SPAN; else if(sx>SPAN/2)b.baseX-=SPAN;
      ...
      // the front row leaves a GAP where a store stands — hide any of its buildings that overlap
      if(b.band.store){ const hide=gaps.some(gx=>Math.abs(nx-gx) < b.mesh.scale.x/2 + STORE_W/2 - 0.4);
        if(b.mesh.visible!==!hide){ b.mesh.visible=!hide; for(const e of b.extras) e.visible=!hide; } } }
